...
export const meta: V2_MetaFunction = () => [{ title: 'CommuniDAO' }, { name: 'color-scheme', content: 'light only' }];
...
export default function App() {
  ...
  const { userStellarAccounts, discordUser, walletAuthed, discordAuthed, provider, account, STELLAR_NETWORK } =
    useLoaderData() ?? {};
  ...
  return (
    <html lang="en" data-theme="dark">
      ...
      <body>
        <WalletProvider walletAuthed={!walletAuthed} publicKey={account} provider={provider} network={STELLAR_NETWORK}>
          <ModalProvider>
            <>
              <Menu discordUser={discordUser} userAccounts={userStellarAccounts} />
              <Outlet />
            </>
          </ModalProvider>
        </WalletProvider>
